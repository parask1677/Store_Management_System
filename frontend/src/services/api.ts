...
export const ApiService = {
  ...
  register: (name: string, email: string, role: 'admin' | 'customer', phone?: string, address?: string) =>
    withFallback(
      () => api.post('/auth/register', { name, email, password: '123456', role, phone, address }),
      () => {
        ...
        // If customer, also save to local storage customers list
        if (role === 'customer') {
